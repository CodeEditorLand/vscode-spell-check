...
export default class SpellProvider implements vscode.CodeActionProvider {
	...
	public activate(context: vscode.ExtensionContext) {
		...
		if (config.get("StopAsking", false)) {
			...
			vscode.window
				.showWarningMessage<UpgradeMessageItem>(
					"The Spell Checker Extension no longer works - please uninstall it and install another one.",
					{
						title: "Show Suggested Replacement",
						id: upgradeAction.Search,
					},
					{
						title: "Don't Ask Again",
						id: upgradeAction.Stop,
		...
		this.addToDictionaryCmd = vscode.commands.registerCommand(
	...
	public provideCodeActions(
		...
		if (match.length >= 2) error = match[1];
					...
					for (let i = 0; i < docProblems.length; i++) {
						...
						if (startPosInFile !== -1) {
							...
							problems.push({
								...
								startChar:
									linesToMistake[numberOfLinesToMistake]
										.length,
								endLine: numberOfLinesToMistake,
								endChar:
									linesToMistake[numberOfLinesToMistake]
										.length + problemTXT.length,
	...
	private removeUnwantedText(content: string): string {
		...
		for (let x = 0; x < expressions.length; x++) {
			...
			if (match !== null) {
				if (DEBUG)
					console.log(
						"Found [" + match.length + "] matches for removal"
					);
